...
#' plot.coreMicrobiome
#'
#' create a venn diagramm of the core microbiomes 
#' 
#'@description create a venn diagramm with the two 
#'              core microbiome of the free living 
#'              and the mariculture HoldingCondition. 
#'              
#'@details The plot can only be saved in a .tiff file!
#' 
#'@param phyloseq   phyloseq object
#'@param file       file name for .tiff
#'
#'@return venn diagramm 
#'@export
#'
plot.coreMicrobiome <- function(phyloseq, file) {
    # create the two microbiomes for free living and mariculture
    free <- get.coreMicrobiome(get.free(phyloseq))
    aqua <- get.coreMicrobiome(get.aqua(phyloseq))
    # build the venn diagramm
    p <- venn.diagram(x = list(
        free = rownames(otu_table(free)),
        aqua = rownames(otu_table(aqua))),
        filename = file, 
        main = "Core Microbiome",
        main.cex = 2, 
        main.fontface = "bold",
        col = c("red","blue"),
        fill = c("red","blue"),
...
#' get.coreTable
#'
#' create a informational data.frame about the core microbiome
#' 
#'@description gather informations about important features of 
#'             the core microbiome and aggregate them in a data.frame. 
#'             This will be:
#'             tax id          - taxonomical identification number 
#'                               of the NCBI taxonomy database
#'             scientific.name - name of the OTU 
#'             tax.rank        - taxonomic rank 
#'             count           - number of occurrance in every sample 
#'                           
#'@details The plot can only be saved in a .tiff file!
#' 
#'@param phyloseq   phyloseq object
#'@param file       file name for .tiff
#'@param col.names  switch colnames on/off
#'@param row.names  switch rownames on/off
#'
#'@return data.frame
#'@export
#'
get.coreTable <- function(phyloseq, 
                          file = NULL, 
                          col.names = F, 
                          row.names = F) {
...
#' and plot them
#'
#'@description calculate rarefaction curves for the core otus 
#'             and plot them
#'
#'@param x              phyloseq object
#'@param n              number of replicates
#'@param steps          rarefaction steps for faster calculation 
#'                      and smoother curves
...
#'@export
#'
plot.rarifyCoreOtus <- function(phyloseq, 
                                n = 25, 
                                steps = 20, 
                                file = NULL, 
                                title = "Core OTUs - Rarefaction curves") {
    
    # create 3 seperate core otus objects with a n number of replicates 
    rare.core.all <- rarefy.coreOtus(phyloseq,
                                     n = n, 
                                     nsteps = steps)
    rare.core.free <- rarefy.coreOtus(get.free(phyloseq), 
                                      n = n, 
                                      nsteps = steps)
    rare.core.aqua <- rarefy.coreOtus(get.aqua(phyloseq), 
                                      n = n, 
                                      nsteps = steps)
    ...
    core.df <- do.call("rbind", list(df.all, 
                                     df.free, 
                                     df.aqua))
    
    # insert a variable for seperation in legend and color
    core.df$group <- factor(core.df$group, 
                            labels = c("E. fuscoguttatus - all", 
