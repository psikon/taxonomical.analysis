...
#' plot.variance
#'
#' simple plot function for the variance structure of OTU's
#' to discover the variance value for DESeq
#' 
#'@param physeq         phyloseq object
...
#'@keywords internal
#'
plot.variance <- function(phyloseq, 
                          x.text = "log10(variance)", 
                          title = "Variance of OTUs" ) {
  hist(log10(apply(otu_table(phyloseq), 1, var)), 
       xlab = x.text, 
       main = title)
}
#' deseq.differential.OTUs
#'
#' import a phyloseq object to deseq and calculate differential
#' appearing OTU's
#' 
#'@description from phyloseq extension tutorial 
#'              (http://joey711.github.io/phyloseq-extensions/DESeq.html)
...
#'@param variance_threshold see documentation
#'@param alpha              see documentation
#'
#'@return significance table
#'@export
#'
deseq.differential.OTUs <- function(phyloseq, 
    ...
    if (nrow(sigtab) == 0) {
        ...
        sigtab <- cbind(as(sigtab, "data.frame"), 
                        as(tax_table(data)[sigtab$id, ], 
                           "matrix"))
        sigtab <- sigtab[order(sigtab$padj), ]
        return(sigtab)
    }
}
#' deseq2.differential.OTUs
#'
#' import a phyloseq object to deseq2 and calculate differential
#' appearing OTU's
#' 
#'@description from phyloseq extension tutorial 
...
#'@export
#'
deseq2.differential.OTUs <- function(phyloseq, 
                                     alpha) {
    
    # convert phyloseq object to deseq2 object
    deseq2 <- suppressWarnings(phyloseq_to_deseq2(rm.underscore(phyloseq), 
                                                  ~HoldingCondition))
...
#' edgeR.differential.OTUs
#'
#' import a phyloseq object to edgeR and calculate differential
#' appearing OTU's
#' 
#'@description from phyloseq extension tutorial 
#'              (http://joey711.github.io/phyloseq-extensions/edgeR.html)
...
#'@export
#'
edgeR.differential.OTUs <- function(phyloseq, 
                                    alpha) {
    
    # convert ophyloseq object to edgeR object
    dge <- phyloseq_to_edgeR(rm.underscore(phyloseq), 
                             "HoldingCondition")
    ...
    tt <- topTags(et, n = nrow(dge$table), 
                  adjust.method = "BH", 
                  sort.by = "PValue")
    res <- tt@.Data[[1]]
    sigtab = res[(res$FDR < alpha),]
    sigtab = cbind(as(sigtab, "data.frame"), 
                   as(tax_table(rm.underscore(phyloseq))[rownames(sigtab), ], 
                      "matrix"))
    return(sigtab)   
}
#' plot.differential.OTU's
#'
#' plot the log2FoldChange of the differential appearing OTUs, 
#' calculated by deseq, deseq2 or edgeR object 
#' 
#'@description from phyloseq extension tutorial 
#'             (http://joey711.github.io/phyloseq-extensions/DESeq2.html)
#' 
#'@param sigtab     significance table
#'@param file       file name for output
#'@param origin     method of sigtab creation
#'
#'@return ggplot2 object
#'@export
#'
plot.differential.OTUs <- function(sigtab, 
                                   file = NULL, 
                                   origin = "deseq") {
    # set up ggplot theme
    theme_set(theme_bw())
    ...
    # check significance table
    if(is.null(sigtab)) {
        stop('significance table is empty')
    }
    
    # create plotting data for DESeq and DESeq2
    if (origin == "deseq") {
        # Phylum order
        ...
        # create a plotting data for edgeR
        sigtab = subset(sigtab, !is.na(genus))
        # Phylum order
        x = tapply(sigtab$logFC, 
                   sigtab$phylum, 
                   function(x) max(x))
        x = sort(x, TRUE)
        sigtab$phylum = factor(as.character(sigtab$phylum), 
