...
get.DBConnection.new <- function(metadataList) {
    ...
    return(list(sample60.new, sample64.new, sample66.new, sample68.new, 
                sample70.new, sample72.new, sample74.new, sample76.new, sample78.new, 
                sample80.new, sample82.new))
}

create.QueryIDFile <- function(path = "data/functional", position = NULL) {
    ...
    files <- lapply(db, function(x) {
        ...
        file <- paste0(path, "/", as.character(x$.metadata["SampleName"]), ".txt")
        # extract the query_ids
        query_ids <- db_query(conn(x),"Select query_def from query", 1)
        # write query_ids to file line by line
        write.table(query_ids, file = file, quote = FALSE, row.names = F, col.names = F)
        # return filepath
        file
    })
    return(files)
...
create.FastaFromTaxonomyReportDB <- function(path= "data/functional", fasta.files, position = NULL) {
    # create the index files
    index.files <- create.QueryIDFile(path = path, position = position)
    for (i in seq_along(index.files)) {
        # resolve filename
        name <- strsplit(strsplit(index.files[[i]], split = "/")[[1]][3], '.', 1)[[1]][1]
        message(paste0("create fasta file for: ", name))
        # construct output path
        output <- paste0(path, "/", name, ".fasta")
        # generate commandline string
        cmd <- paste0("python src/fastaExtractor.py --header ", index.files[[i]], " --fasta ", fasta.files[i], " --output ", output)
