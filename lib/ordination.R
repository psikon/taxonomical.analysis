...
plot_ordination_Samples <- function(phyloseq, file = "graphs/ordination/ordinate.samples.pdf",
    ...
    phyloseq <- get_rarefied_phyloseq(bakteria, 1234,replace = T, trimOTUs = T)
    hits = 5
    num_samples = 0.5
    phyloseq
    level = "phylum"
    ...
    toplevel = compactNA(names(sort(level.sum, TRUE))[1:num_best])
    phyloseq = prune_taxa((tax_table(phyloseq)[, level] %in% toplevel), phyloseq)
    phyloseq <- tax_glom(phyloseq, "phylum")
    phyloseq
    
    theme_set(theme_bw())
    phyloseq.ord <- ordinate(phyloseq, method = "MDS", distance = "bray")
    p = plot_ordination(phyloseq, phyloseq.ord, type = "samples", color = "SampleName",shape = "HoldingCondition")
